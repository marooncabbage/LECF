...
class Hyperboloid(Manifold):
    ...
    def minkowski_dot(self, x, y, keepdim=True): # hgcf.equ (1)
        res = torch.sum(x * y, dim=-1) - 2 * x[..., 0] * y[..., 0]# https://ask.hellobi.com/blog/wangdawei/36627
        if keepdim:
    ...
    def sqdist(self, x, y, c): # hgcf.equ (2)
        K = 1. / c
    ...
    def proj(self, x, c):  # Nickel.equ (6)
        K = 1. / c
        d = x.size(-1) - 1
        y = x.narrow(-1, 1, d)
    ...
    def proj_tan(self, u, x, c): ## Nickel.equ (10.5)
        d = x.size(1) - 1
    ...
    def expmap(self, u, x, c): # Nickel.equ (9)
        K = 1. / c
        sqrtK = K ** 0.5
    ...
    def expmap0(self, u, c):# hgcf.equ (6)
        K = 1. / c
        sqrtK = K ** 0.5
        d = u.size(-1) - 1
    ...
    def logmap0(self, x, c): # hgcf.equ (8)
        K = 1. / c
        sqrtK = K ** 0.5
        d = x.size(-1) - 1
