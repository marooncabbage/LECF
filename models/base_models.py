...
class LECFModel(nn.Module):

    def __init__(self, users_items, args):
        """
        the learnable parameters are the input coordinate x is self
        Parameters
        ----------
        users_items
        args
        """
        super(LECFModel, self).__init__()

        self.c = torch.tensor([args.c]).to(default_device(args))
        self.manifold = getattr(manifolds, "Hyperboloid")()
        self.nnodes = args.n_nodes
        self.encoder = getattr(encoders, "LECF")(self.c, args)
        ...
        self.embedding = nn.Embedding(num_embeddings=self.num_users + self.num_items,
                                      embedding_dim=args.embedding_dim).to(default_device(self.args))
    ...
    def encode(self, adj):
        ...
        if torch.cuda.is_available():
           adj = adj.to(default_device(self.args))
           x = x.to(default_device(self.args))
