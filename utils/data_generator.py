...
class Data(object):
    """
    self.adj_train : the training uer-item adj in csr
    """
    def __init__(self, dataset, norm_adj, seed, test_ratio):
        ...
        if dataset == 'ml-100k':
            ...
            self.train_dict, self.test_dict = self.split_data_randomly(self.user_item_list, test_ratio, seed)   # key :user ,value : item ,split for every user
        ...
        self.adj_train, user_item = self.generate_adj() # adj_train : https://imgur.com/Lca8mDt
        ...
        print(f'adjacency matrix shape: {self.adj_train.shape}')

        tot_num_rating = sum([len(x) for x in self.user_item_list])

        print(f'number of all ratings: {tot_num_rating}, density: { tot_num_rating / (self.num_users * self.num_items):.6f}')
    ...
    def generate_adj(self):
        ...
        for i, v in self.train_dict.items():
            user_item[i][v] = 1
            #print(f'v: {v}####')
        coo_user_item = sp.coo_matrix(user_item)   # sparse matrix in COOrdinate format. coo_matrix((data, (row, col))


        print('generating adj csr... ')
        start = time.time()
        rows = np.concatenate((coo_user_item.row, coo_user_item.transpose().row + self.num_users))
        cols = np.concatenate((coo_user_item.col + self.num_users, coo_user_item.transpose().col)) #transfer to bidirection matrix
        data = np.ones((coo_user_item.nnz * 2,))
        adj_csr = sp.coo_matrix((data, (rows, cols))).tocsr().astype(np.float32)   # csr : Compressed Sparse Row matrix
