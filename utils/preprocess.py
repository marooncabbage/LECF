...
def remove_infrequent_items(data, min_counts=5):
    ...
    print(f" {len(counts >= min_counts)} items with < {min_counts} interactoins are removed")
    return df
...
def remove_infrequent_users(data, min_counts=10):
    ...
    print(f"{len(counts >= min_counts)} users with < {min_counts} interactoins are removed")
    return df
...
if __name__ == "__main__":
    ...
    data_records = read_user_rating_records()   # get the original data file
    data_records.loc[data_records.rating < 4, 'rating'] = 0
    data_records.loc[data_records.rating >= 4, 'rating'] = 1
    data_records = data_records[data_records.rating > 0]   # only reserve the items which rating=1 from the original data
    ...
    data = filtered_data.groupby('user_id')['item_id'].apply(list)   # (in this is the group) [in this is the order] # https://realpython.com/pandas-groupby/#pandas-groupby-putting-it-all-together
    unique_data = filtered_data.groupby('user_id')['item_id'].nunique()
    data = data[unique_data[unique_data >= params[1]].index]

    user_item_dict = data.to_dict()   # the final mainly data
    user_mapping = []
    item_set = set()
    ...
    print(f'num of users:{len(user_mapping)}, num of items:{len(item_mapping)}')
    ...
    print(f" numbers of stored values in rating_matrix:{rating_matrix.nnz}")
