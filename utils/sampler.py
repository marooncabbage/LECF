...
def sample_function(adj_train, num_nodes, batch_size, n_negative, result_queue):
    ...
    user_item_pairs = all_pairs[: adj_train.count_nonzero() // 2] #[[rows],[columns]].T
    ...
    while True:
        for i in range(int(len(user_item_pairs) / batch_size)):
            ...
            user_negative_samples = np.random.randint(num_users, sum(num_nodes), size=(samples_for_users, n_negative)) # get the negtive samples
            ...
            for user_positive, negatives, i in zip(user_positive_items_pairs,
                ...
                for j, neg in enumerate(negatives):
                    while neg in user_item_pairs_set[user]:
                        #print("neg",neg)
                        user_negative_samples[i, j] = neg = np.random.randint(num_users, sum(num_nodes)) # exclude the positive pairs
...
class WarpSampler(object):
    ...
    * learning tutorials of multiprocessing :
