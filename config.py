...
config_args = {
    'training_config': {
        'log': ("log1", 'None for no logging'),
        'lr': (0.001, 'learning rate'),
        'batch-size': (10000, 'batch size'),
        'cuda': (0, 'which cuda device to use (-1 for cpu training)'),
        'epochs': (12000, 'maximum number of epochs to train for'),
    ...
    'model_config': {
        ...
        'model_path': ("saved-model", 'margin value in the metric learning loss'),
    },
    'data_config': {
        'dataset': ('Amazon-CD', 'which dataset to use'), #Amazon-CD,Amazon-Book,yelp
